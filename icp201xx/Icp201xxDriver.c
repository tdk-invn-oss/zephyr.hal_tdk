...
int inv_icp201xx_wr_pefe_gain_trim(inv_icp201xx_serif_t *s, uint8_t new_value)
{
	...
	reg_value =
		(reg_value & (~BIT_PEFE_GAIN_TRIM_MASK)) | (new_value << BIT_PEFE_GAIN_TRIM_POS);
...
int inv_icp201xx_wr_otp_write_switch(inv_icp201xx_serif_t *s, uint8_t new_value)
{
	...
	reg_value = (reg_value & (~BIT_OTP_CONFIG1_WRITE_SWITCH_MASK)) |
		    (new_value << BIT_OTP_CONFIG1_WRITE_SWITCH_POS);
...
int inv_icp201xx_wr_otp_dbg2_reset(inv_icp201xx_serif_t *s, uint8_t new_value)
{
	...
	reg_value =
		(reg_value & (~BIT_OTP_DBG2_RESET_MASK)) | (new_value << BIT_OTP_DBG2_RESET_POS);
...
int inv_icp201xx_wr_forced_meas_trigger(inv_icp201xx_serif_t *s,
					icp201xx_forced_meas_trigger_t new_value)
{
	int status;
	...
	reg_value = (reg_value & (~BIT_FORCED_MEAS_TRIGGER_MASK)) |
		    (new_value << BIT_FORCED_MEAS_TRIGGER_POS);
	do {
		inv_icp201xx_rd_mode_sync_status(s, &read_value);
		if (read_value) {
			break;
...
int inv_icp201xx_rd_forced_meas_trigger(inv_icp201xx_serif_t *s,
					icp201xx_forced_meas_trigger_t *value)
{
	int status;
	uint8_t reg_value;
	status = inv_icp201xx_serif_read_reg(s, MPUREG_MODE_SELECT, 1, &reg_value);
	...
	*value = (icp201xx_forced_meas_trigger_t)((reg_value & BIT_FORCED_MEAS_TRIGGER_MASK) >>
...
int inv_icp201xx_rd_meas_mode(inv_icp201xx_serif_t *s, icp201xx_meas_mode_t *value)
{
	...
	*value = (icp201xx_meas_mode_t)((reg_value & BIT_MEAS_MODE_MASK) >>
					BIT_FORCED_MEAS_MODE_POS);
...
int inv_icp201xx_rd_pow_mode(inv_icp201xx_serif_t *s, icp201xx_power_mode_t *value)
{
	...
	*value = (icp201xx_power_mode_t)((reg_value & BIT_POWER_MODE_MASK) >>
					 BIT_FORCED_POW_MODE_POS);

	return status;
}

int inv_icp201xx_wr_fifo_readout_mode(inv_icp201xx_serif_t *s,
				      icp201xx_FIFO_readout_mode_t new_value)
{
	int status;
	uint8_t reg_value = 0, read_value = 0;
	status = inv_icp201xx_serif_read_reg(s, MPUREG_MODE_SELECT, 1, &reg_value);
...
int inv_icp201xx_rd_lp_seq_sync_status(inv_icp201xx_serif_t *s, uint8_t *value)
{
	...
	*value = (read_val & BIT_DEVICE_STATUS_LP_SEQ_STATE_MASK) >>
		 BIT_DEVICE_STATUS_LP_SEQ_STATE_POS;
