 ...
 * @param[in] serif Pointer to serial interface struct updated with read,write function pointers and
 * interface mode. * @return    0 in case of success, negative value on error. See enum inv_status
 */
int INV_EXPORT inv_icp201xx_init(inv_icp201xx_t *s, const inv_icp201xx_serif_t *serif);

/** @brief Configure ICP201xx to Start/Stop Pressure and/or Temperature measurements.( All other
 * required fields are set to default values )
 * @param[in] s Pointer to the driver context structure inv_icp201xx_t
 * @param[in] op_mode  Operation mode to configure BW and ODR * @param[in] fifo_read_mode FIFO read
 * out mode configuration.
 * @return    0 in case of success, negative value on error. See enum inv_status
 */
int INV_EXPORT inv_icp201xx_config(inv_icp201xx_t *s, icp201xx_op_mode_t op_mode,
				   icp201xx_FIFO_readout_mode_t fifo_read_mode);
 ...
 * @return    0 in case of success, negative value on error. See enum inv_status
 */
int INV_EXPORT inv_icp201xx_adv_config(inv_icp201xx_t *s, icp201xx_op_mode_t op_mode,
 ...
 *          Other configuration ( FIFO, FIFO read out mode, mode select, int_mask etc  ) are not
 * changed. * @param[in] s Pointer to the driver context structure inv_icp201xx_t
 * @return    0 in case of success, negative value on error. See enum inv_status
 */
 ...
 * @param[in] s Pointer to the driver context structure inv_icp201xx_t
 * @param[in] press_int_mask interrupt mask for Pressure sensor( need to be set with
 * ICP201XX_INT_MASK_PRESS_XXXXX)
 * @param[in] press_abs  Pressure overrun/underrun value. Valid only if ICP201XX_INT_MASK_PRESS_ABS
 * is set in press_int_mask interrupt.
 * @param[in] press_delta  Delta pressure overrun/underrun value. Valid only if
 * ICP201XX_INT_MASK_PRESS_DELTA is set in press_int_mask interrupt.
 * @return    0 in case of success, negative value on error. See enum inv_status
 */
int INV_EXPORT inv_icp201xx_set_press_notification_config(inv_icp201xx_t *s, uint8_t press_int_mask,
							  int16_t press_abs, int16_t press_delta);

/** @brief Read Pressure sensor notifications from ICP201xx.
 * @param[in] s Pointer to the driver context structure inv_icp201xx_t
 * @param[out] press_int_mask interrupt mask for Pressure sensor( need to be set with
 * ICP201XX_INT_MASK_PRESS_XXXXX)
 * @param[out] press_abs  Pressure overrun/underrun value. Valid only if ICP201XX_INT_MASK_PRESS_ABS
 * is set in press_int_mask interrupt.
 * @param[out] press_delta  Delta pressure overrun/underrun value. Valid only if
 * ICP201XX_INT_MASK_PRESS_DELTA is set in press_int_mask interrupt.
 * @return    0 in case of success, negative value on error. See enum inv_status
 */
int INV_EXPORT inv_icp201xx_get_press_notification_config(inv_icp201xx_t *s,
							  uint8_t *press_int_mask,
							  int16_t *press_abs, int16_t *press_delta);

/** @brief Config FIFO notifications.
 * @param[in] s Pointer to the driver context structure inv_icp201xx_t
 * @param[in] fifo_int_mask interrupt mask ( need to be set with ICP201XX_INT_MASK_FIFO_XXXXX)
 * @param[in] fifo_wmk_high  FIFO high watermark value. Valid only if
 * ICP201XX_INT_MASK_FIFO_WMK_HIGH is set in fifo_int_mask. *                             if set to
 * 0 high watermark check is disabled.
 * @param[in] fifo_wmk_low  FIFO Low watermark value. Valid only if ICP201XX_INT_MASK_FIFO_WMK_LOW
 * is set in fifo_int_mask.
 * @return    0 in case of success, negative value on error. See enum inv_status
 */
int INV_EXPORT inv_icp201xx_set_fifo_notification_config(inv_icp201xx_t *s, uint8_t fifo_int_mask,
 ...
 * @param[out] fifo_wmk_high  FIFO high watermark value. Valid only if
 * ICP201XX_INT_MASK_FIFO_WMK_HIGH is set in fifo_int_mask. *                             if set to
 * 0 high watermark check is disabled.
 * @param[out] fifo_wmk_low  FIFO Low watermark value. Valid only if ICP201XX_INT_MASK_FIFO_WMK_LOW
 * is set in fifo_int_mask.
 * @return    0 in case of success, negative value on error. See enum inv_status
 */
int INV_EXPORT inv_icp201xx_get_fifo_notification_config(inv_icp201xx_t *s, uint8_t *fifo_int_mask,
							 uint8_t *fifo_wmk_high,
							 uint8_t *fifo_wmk_low);
 ...
 * @param[in] s Pointer to the driver context structure inv_icp201xx_t
 * @param[out] dev_status current device status as read from mems * @return    0 in case of success,
 * negative value on error. See enum inv_status
 */
...
/** @brief Reads up to requested number of packets from FIFO. * @param[in] s Pointer to the driver
 * context structure inv_icp201xx_t
 ...
 * @return    0 in case of success, negative value on error. See enum inv_status
 */
int INV_EXPORT inv_icp201xx_process_raw_data(inv_icp201xx_t *s, uint8_t packet_cnt, uint8_t *data,
...
/** @brief Get Device ID and version from ICP201xx * @param[in] s Pointer to the driver context
 * structure inv_icp201xx_t
 * @param[in] pointer to device id.
 * @param[in] pointer to version.
 * @return    0 in case of success( device is validated), negative value on error. See enum
 * inv_status
 */
