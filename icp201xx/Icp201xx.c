...
int inv_icp201xx_config(inv_icp201xx_t *s, icp201xx_op_mode_t op_mode,
			icp201xx_FIFO_readout_mode_t fifo_read_mode)
{
	...
	status |= inv_icp201xx_wr_mode_select(&(s->serif),
					      reg_value); /* Set Sensor in Standby mode */
...
int inv_icp201xx_adv_config(inv_icp201xx_t *s, icp201xx_op_mode_t op_mode,
...
{
	return inv_icp201xx_wr_forced_meas_trigger(&(s->serif),
						   ICP201XX_FORCE_MEAS_TRIGGER_FORCE_MEAS);
	;
}
...
int inv_icp201xx_set_press_notification_config(inv_icp201xx_t *s, uint8_t press_int_mask,
					       int16_t press_abs, int16_t press_delta)
{
	...
	reg_value = (reg_value | (ICP201XX_INT_MASK_PRESS_ABS | ICP201XX_INT_MASK_PRESS_DELTA)) &
		    ~press_int_mask;
	...
	if (press_int_mask & ICP201XX_INT_MASK_PRESS_ABS) {
		...
		status |= inv_icp201xx_wr_press_abs_msb(&(s->serif),
							(uint8_t)((press_abs >> 8) & 0xff));
	} else {
	...
	if (press_int_mask & ICP201XX_INT_MASK_PRESS_DELTA) {
		status |=
			inv_icp201xx_wr_press_delta_lsb(&(s->serif), (uint8_t)(press_delta & 0xff));
		status |= inv_icp201xx_wr_press_delta_msb(&(s->serif),
							  (uint8_t)((press_delta >> 8) & 0xff));
	} else {
...
int inv_icp201xx_get_press_notification_config(inv_icp201xx_t *s, uint8_t *press_int_mask,
					       int16_t *press_abs, int16_t *press_delta)
{
	...
	*press_int_mask =
		(reg_value & (ICP201XX_INT_MASK_PRESS_ABS | ICP201XX_INT_MASK_PRESS_DELTA));
...
int inv_icp201xx_set_fifo_notification_config(inv_icp201xx_t *s, uint8_t fifo_int_mask,
					      uint8_t fifo_wmk_high, uint8_t fifo_wmk_low)
{
	...
	status |= inv_icp201xx_rd_int_mask(&(s->serif), &reg_value);
	reg_value =
		(reg_value | (ICP201XX_INT_MASK_FIFO_WMK_HIGH | ICP201XX_INT_MASK_FIFO_OVER_FLOW |
			      ICP201XX_INT_MASK_FIFO_WMK_LOW | ICP201XX_INT_MASK_FIFO_UNDER_FLOW)) &
		~fifo_int_mask;
...
int inv_icp201xx_get_fifo_notification_config(inv_icp201xx_t *s, uint8_t *fifo_int_mask,
					      uint8_t *fifo_wmk_high, uint8_t *fifo_wmk_low)
{
	uint8_t reg_value = 0;
	int status = INV_ERROR_SUCCESS;
...
int inv_icp201xx_get_fifo_data(inv_icp201xx_t *s, uint8_t req_packet_cnt, uint8_t *data)
{
	uint8_t fifo_read_offset =
		((s->fifo_readout_mode == ICP201XX_FIFO_READOUT_MODE_PRES_ONLY) ||
		 (s->fifo_readout_mode == ICP201XX_FIFO_READOUT_MODE_TEMP_ONLY))
			? 3
			: 0;
	uint8_t packet_cnt = ((s->fifo_readout_mode == ICP201XX_FIFO_READOUT_MODE_PRES_ONLY) ||
			      (s->fifo_readout_mode == ICP201XX_FIFO_READOUT_MODE_TEMP_ONLY))
				     ? (req_packet_cnt * 1 * 3)
				     : (req_packet_cnt * 2 * 3);
				  ...
				  int32_t *pressure, int32_t *temperature)
{
	...
	for (i = 0; i < packet_cnt; i++) {
		if (s->fifo_readout_mode == ICP201XX_FIFO_READOUT_MODE_PRES_TEMP) {
			pressure[i] = (int32_t)(((data[offset + 2] & 0x0f) << 16) |
						(data[offset + 1] << 8) | data[offset]);
			offset += 3;
			temperature[i] = (int32_t)(((data[offset + 2] & 0x0f) << 16) |
						   (data[offset + 1] << 8) | data[offset]);
			offset += 3;
