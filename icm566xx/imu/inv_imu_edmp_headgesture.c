...
#include "imu/edmp_ram_hdgst_image.h"
	...
	status |= inv_imu_write_sram(s, EDMP_RAM_HDGST_DATA_BASE, sizeof(ram_img), ram_img);
					    ...
					    inv_imu_edmp_headgesture_int_state_t *it)
{
	...
	status |= inv_imu_write_reg(s, INT_APEX_CONFIG0, 1, (uint8_t *)&int_apex_config0);
					    ...
					    inv_imu_edmp_headgesture_int_state_t *it)
{
	...
	status |= inv_imu_read_reg(s, INT_APEX_STATUS0, 1, (uint8_t *)&int_apex_status0);
