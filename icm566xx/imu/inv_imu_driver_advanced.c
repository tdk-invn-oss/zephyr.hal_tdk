				   ...
				   uint16_t *fifo_count)
{
	...
	/* Read FIFO count */
	status |= inv_imu_read_reg(s, FIFO_COUNT_0, 2, &fifo_count_reg[0]);
...
int inv_imu_adv_enable_clkin_rtc(inv_imu_device_t *s)
{
	 ...
	 */
	status |= inv_imu_read_reg(s, IPREG_SYS2_REG_109, 1, (uint8_t *)&ipreg_sys2_reg_109);
	ipreg_sys2_reg_109.accel_src_ctrl = IPREG_SYS2_REG_109_ACCEL_SRC_CTRL_SRC_ON_PREFILTER_ON;
	status |= inv_imu_write_reg(s, IPREG_SYS2_REG_109, 1, (uint8_t *)&ipreg_sys2_reg_109);

	status |= inv_imu_read_reg(s, IPREG_SYS1_REG_154, 1, (uint8_t *)&ipreg_sys1_reg_154);
	ipreg_sys1_reg_154.gyro_src_ctrl = IPREG_SYS1_REG_154_GYRO_SRC_CTRL_SRC_ON_PREFILTER_ON;
	status |= inv_imu_write_reg(s, IPREG_SYS1_REG_154, 1, (uint8_t *)&ipreg_sys1_reg_154);

	status |= inv_imu_read_reg(s, OTP_HEATER_RTC_CONFIG, 1, (uint8_t *)&otp_heater_rtc_config);
	otp_heater_rtc_config.rtc_mode = INV_IMU_ENABLE;
	status |= inv_imu_write_reg(s, OTP_HEATER_RTC_CONFIG, 1, (uint8_t *)&otp_heater_rtc_config);
