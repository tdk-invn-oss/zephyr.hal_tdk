...
int inv_imu_set_gyro_ln_bw(inv_imu_device_t *s, ipreg_sys1_reg_158_gyro_ui_lpfbw_sel_t gyr_bw)
{
	int status = INV_IMU_OK;
	...
	status |= inv_imu_write_reg(s, IPREG_SYS1_REG_158, 1, (uint8_t *)&ipreg_sys1_reg_158);

	return status;
}

int inv_imu_set_gyro_lp_avg(inv_imu_device_t *s, ipreg_sys1_reg_157_gyro_lp gyr_avg)
{
	int status = INV_IMU_OK;
	ipreg_sys1_reg_157_t ipreg_sys1_reg_157;

	status |= inv_imu_read_reg(s, IPREG_SYS1_REG_157, 1, (uint8_t *)&ipreg_sys1_reg_157);
	ipreg_sys1_reg_157.gyro_lp_avg_sel = gyr_avg;
	status |= inv_imu_write_reg(s, IPREG_SYS1_REG_157, 1, (uint8_t *)&ipreg_sys1_reg_157);
	return status;
}

int inv_imu_set_accel_lp_avg(inv_imu_device_t *s, ipreg_sys2_reg_110_accel_lp_avg_sel_t acc_avg)
{
	int status = INV_IMU_OK;
	ipreg_sys2_reg_110_t ipreg_sys2_reg_110;

	status |= inv_imu_read_reg(s, IPREG_SYS2_REG_110, 1, (uint8_t *)&ipreg_sys2_reg_110);
	ipreg_sys2_reg_110.accel_lp_avg_sel = acc_avg;
	status |= inv_imu_write_reg(s, IPREG_SYS2_REG_110, 1, (uint8_t *)&ipreg_sys2_reg_110);

	return status;
}

int inv_imu_set_accel_ln_bw(inv_imu_device_t *s, ipreg_sys2_reg_112_accel_ui_lpfbw_t acc_bw)
{
	int status = INV_IMU_OK;
	...
	status |= inv_imu_write_reg(s, IPREG_SYS2_REG_112, 1, (uint8_t *)&ipreg_sys2_reg_112);

	return status;
}

int inv_imu_set_gyro_notch_bypass(inv_imu_device_t *s, uint8_t bypass_gyr_notch)
{
	int status = INV_IMU_OK;
	ipreg_sys1_reg_157_t ipreg_sys1_reg_157;
...
int inv_imu_get_register_data(inv_imu_device_t *s, inv_imu_sensor_data_t *data)
{
	...
	status |= inv_imu_read_reg(s, ACCEL_DATA_X_0, sizeof(high_order_data), high_order_data);
#ifdef EXT_DATA_X
...
int inv_imu_select_accel_lp_clk(inv_imu_device_t *s, pwr_mgmt_0_accel_lp_clk_sel_t clk_sel)
{
	int status = INV_IMU_OK;
	pwr_mgmt0_t pwr_mgmt0;
