...
int icm566xx_get_register_data(inv_imu_device_t *s, inv_imu_sensor_data_t *data)
{
	...
	FORMAT_SENSOR_DATA(s->endianness_data, (uint8_t *)&high_order_data[0], (low_order_data[0] & 0x0F),
					   &data->accel_data[0]);
	FORMAT_SENSOR_DATA(s->endianness_data, (uint8_t *)&high_order_data[2], (low_order_data[1] & 0x0F),
					   &data->accel_data[1]);
	FORMAT_SENSOR_DATA(s->endianness_data, (uint8_t *)&high_order_data[4], (low_order_data[2] & 0x0F),
					   &data->accel_data[2]);
	/* Format gyro data from sensor registers. */
	FORMAT_SENSOR_DATA(s->endianness_data, (uint8_t *)&high_order_data[6], ((low_order_data[0] >> 4) & 0x0F),
					   &data->gyro_data[0]);
	FORMAT_SENSOR_DATA(s->endianness_data, (uint8_t *)&high_order_data[8], ((low_order_data[1] >> 4) & 0x0F),
					   &data->gyro_data[1]);
	FORMAT_SENSOR_DATA(s->endianness_data, (uint8_t *)&high_order_data[10], ((low_order_data[2] >> 4) & 0x0F),
					   &data->gyro_data[2]);
