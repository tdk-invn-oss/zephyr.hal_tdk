 ...
 * Default value: 2080374 (corresponding to about 62mgee)
 * Format: int32_t
 */
 ...
 * Range: [0 - 225]
 * Default value: 150 for ODR = 50 Hz (corresponding to 3s)
...
/* ped_low_en_amp_th
 *
 * Threshold on signal amplitude on filtered accelerometer to identify a valid step in slow walk mode. Used to increase step detection for slow walk use case only.
 * Unit: LSB with 1 LSB = 1 g/2^25
 * Range: [1006632 - 3523215]
 * Default value: 2684354 (corresponding to about 80mgee)
 * Format: int32_t
 */
...
/* r2w_sleep_time_out
 *
 * Defines the duration after wake event to report sleep event regardless if position changed or not.
 * Unit: Time in ms, not ODR dependent
 * Default value: 640 (corresponding to 0.64s)
 * Range: [100 - 10000]
 * Format: int32_t
 */
 ...
 *
 * Timer relative to the rapidity of the algorithm to trigger wake up when the orientation before motion is Y axis up (with less than 30 degree of inclination)
 * Unit: Time in ms (not ODR dependent)
 * Default value: 240
 * Range: [100 - 500]
 * Format: int32_t
 ...
 *
 * Timer relative to the rapidity of the algorithm to trigger wake up when the orientation before motion is over 30 degree on the Y axis
 * Unit: Time in ms (not ODR dependent)
 * Default value: 500
 * Range: [240- 1000]
 * Format: int32_t
 ...
 *
 * Time delay to update gravity in case motion is detected continuously, forces to update gravity estimator even if the device is not stable
 * Unit: Time in ms (not ODR dependent)
 * Default value: 2600
 * Range: [1000- 10000]
 * Format: int32_t
...
/* lowg_peak_th
 *
 * If the sum of absolute accelerometer axes falls below lowg_peak_th, this triggers the start of LowG state
 * Unit: in LSB, with 1 LBS = 1g / 2^12
 * Default value: 2048 (corresponding to 0.5g)
 * Range: [128 - 4096]
 * Format: int16_t
 */
 ...
 * Unit: in sample number - ODR dependent
 * Default value: 13 (corresponding to 32ms at 400Hz)
 * Range: [1 - 300]
 * Format: int16_t
 */
...
/* highg_peak_th
 *
 * Any of the absolute accelerometer axis surpasses high_peak_thres will trigger the start of HighG state
 * Unit: in LSB, with 1 LBS = 1g / 2^12
 * Default value: 29696 (corresponding to 7.25g)
 * Range: [1024 - 32768]
 */
...
/* highg_peak_th_hyst
 *
 * A hysteresis high_peak_thres_hyst is subtracted from the threshold (high_peak_thres) after the initial threshold is met
 * Unit: in LSB, with 1 LBS = 1g / 2^12
 * Default value: 640  (corresponding to 0.1563g)
 * Range: [128 - 1024]
 * Format: int16_t
 */
...
/* highg_time_th
 *
 * The number of samples the device should stay above (highg_peak_th - highg_peak_th_hyst) before HighG state is triggered
 * Unit: in sample number - ODR dependent
 * Default value: 1 (corresponding to 2.5ms at 400Hz)
 * Range: [1 - 300]
 * Format: int16_t
 */
...
/* ff_min_duration
 *
 * Minimum duration between a LowG and HighG event to trigger freefall. Shorter freefalls are ignored.
 * Unit: in sample number, ODR dependent
...
/* ff_max_duration
 *
 * Maximum duration after a LowG event to wait HighG event before reset freefall algorithm. Longer freefalls are ignored.
 * Unit: in sample number, ODR dependent
 * Default value: 285 (corresponding to 712ms at 400Hz)
 * Range: [12 - 1040]
 * For information, it is corresponding to approximately a fall of 2.5 meters
 * Format: int32_t
 */
 ...
 *
 * Maximum time spent over tap_max_peak_tol before rejecting the tap candidate, at the end of tap_tmin window.
 * If internal counter of number of samples over the smudge threshold is below the tap_smudge_reject_th, single-tap is detected.
 * Unit: in sample number, ODR dependent
 ...
 * Range: [1 - 63] 
 * Default value: 32 (algorithm will detect only -Z tap, also called backtap)
 ...
 *
 * Threshold for maximal energy on tap primary axis which is used to check validity of the tap detection
 * Unit: N/A
 * Range: [1000 - 10000]
 * Default value: 3000
 * Note: Use 0 to disable this check and it is only valid for tap directional rejection
 * Format: int16_t
 */
 ...
 *
 * Threshold for maximal energy on tap secondary axis which is used to check validity of the tap detection
 * Unit: N/A
 * Range: [200 - 1000]
 * Default value: 500
 * Note: Use 0 to disable this check and it is only valid for tap directional rejection
...
/* double_tap_timing
 *
 * Number of samples between the first and second pulse detected in the detection window, only valid for double tap events
 * Unit: time in sample number
 * Format: uint8_t
 */
...
/* triple_tap_timing
 *
 * Number of samples between the first and third pulse detected in the detection window, only valid for triple tap events
 * Unit: time in sample number
 * Format: uint8_t
 */
...
/* b2s_mounting_matrix
 *
 * Mounting matrix to apply to the accelerometer data before bring-to-see computation, as a bit-mask combination of operations. 3bits are used [b2 b1 b0]:
 * Range: 3 lower bits only are used [b2 b1 b0]:
...
/* b2s_thr_cos_angle
 *
 * Reverse bring-to-see threshold, condition satisfied when moving away from bring-to-see orientation position
 ...
 *
 * Value representing which axis has the majority of nomotion detection: 
 * 0: -x, 1: -y, 2: -z, 3: N/A, 4: +x, 5: +y, 6: +z
 * Format: uint8_t
 */
 ...
 *
 * Axis detection threshold added before comparison between two axis to select the majority axis in done
 * using this formula `majority_axis_nb > minority_axis_nb + shake_axis_majority_th`
 * Unit: no unit, integer value
 * Default value: is 1
 * Format: int16_t
 */
 ...
 * can be set higher to allow the static detection when user in movement (like medium walking, running)
 * Format: int16_t
 */
...
/* shake_thr_motion
 *
 * Threshold to get out of the static phase and detect any motion (not specific to shake), common to all axes.
 * Unit: in LSB, with 1 LBS = 1g / 2^12
 * Default value: 2400 (corresponding to 585mg)
 * Recommendation: can be set lower to be more restrictive on motion situation,
 * can be set higher to allow keep in the static phase when user in movement (like medium walking, running)
 * Format: int16_t
 */
 ...
 *
 * Internal time window for feature extraction to classify shake in progress once motion is internally detected
 * Unit: in sample number, warning ODR dependency
 * Default value: 8, corresponding to 160ms at 50Hz
 * Recommendation: use range [8-16]
 * Format: int16_t
 */
...
/* flat_wait_time
 *
 * Wait time after the flat/no flat angle is reached and maintained to trigger the flat/no flat event
 * Unit: in sample number - ODR dependent
 * Default value: 10 (corresponding to 200ms at 50Hz)
 * Format: int16_t
 */
