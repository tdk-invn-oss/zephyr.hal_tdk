	...
	if (!st_params->accel_en) {
		...
		st_output->accel_status =
		    ((stc_results & (STC_RESULTS_ACCEL_X_MASK | STC_RESULTS_ACCEL_Y_MASK |
                ...
                INV_IMU_ST_STATUS_SUCCESS : INV_IMU_ST_STATUS_FAIL;
							...
							float phase2_out[])
{
	...
	for (int i = 0; i < 3; i++) {
		phase2_out[i] = (float)phase2_total[i] / (float)num_samples_accel[i];		/* Calculate accel data average */
		phase2_out[i+3] = (float)phase2_total[i+3] / (float)num_samples_gyro;		/* Calculate gyro data average */
	}
	return rc;
}
                            ...
                            inv_imu_selftest_output_t *st_output)
{
	...
	if (!(st_params->accel_en || st_params->gyro_en))		/* If neither sensor is enabled, don't run self-test */
		return INV_IMU_ERROR;
	...
	for (int i = 0; i < 3; i++) {
		str_ft_float[i] = (float)str_ft[i] / (float)(1 << 13);			/* accel_{x/y/z} has a 2^13 resolution */
		str_ft_float[i+3] = (float)str_ft[i+3] / (float)(1 << 5);		/* gyro_{x/y/z} has a 2^5 resolution */
	}
	...
	rc |= st_direct_phase1(s, st_params, phase1_out);		/* Run phase1, collect data in normal mode */
	
	rc |= st_direct_phase2(s, st_params, phase2_out);		/* Run phase2, collect data in selftest mode */
	...
	for (int i = 0; i < 3; i++) {
		...
		st_response_float[i] = ((phase2_out[i] - phase1_out[i]) * ACCEL_FS_VALUE) / REG_SAT_VALUE;	
		
		/* Convert delta of the gyro data from the two phases into SI units */		  
		st_response_float[i+3] = ((phase2_out[i+3] - phase1_out[i+3]) * GYRO_FS_VALUE) / REG_SAT_VALUE;  
	}
	...
	if (st_params->accel_en) {
		st_output->ax_status = ((st_response_float[0] < acc_str_min[0]) || (st_response_float[0] > acc_str_max[0])) ? -1 : 1;
		st_output->ay_status = ((st_response_float[1] < acc_str_min[1]) || (st_response_float[1] > acc_str_max[1])) ? -1 : 1;
		st_output->az_status = ((st_response_float[2] < acc_str_min[2]) || (st_response_float[2] > acc_str_max[2])) ? -1 : 1;
		st_output->accel_status = ((st_output->ax_status == 1) && (st_output->ay_status == 1) && 
		                           (st_output->az_status == 1)) ? 1 : -1;
	}
	if (st_params->gyro_en) {
		st_output->gx_status = ((st_response_float[3] < gyr_str_min[0]) || (st_response_float[3] > gyr_str_max[0])) ? -1 : 1;
		st_output->gy_status = ((st_response_float[4] < gyr_str_min[1]) || (st_response_float[4] > gyr_str_max[1])) ? -1 : 1;
		st_output->gz_status = ((st_response_float[5] < gyr_str_min[2]) || (st_response_float[5] > gyr_str_max[2])) ? -1 : 1;
		st_output->gyro_status = ((st_output->gx_status == 1) && (st_output->gy_status == 1) &&
								  (st_output->gz_status == 1)) ? 1 : -1;
	}
	rc |= icm566xx_adv_device_reset(s);
